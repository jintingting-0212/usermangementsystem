...
public class UserDaoImpl implements IUserDao {
	...
	public List<User> findAllUsers(User user) {
		...
		try {
			if(userName.equals("admin")) {
				...
				users = session.selectList(namespace + "findUser", user);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public User findUser(User user) {
		...
		SqlSession session = MyBatisUtil.getSession();
		try {
			users = session.selectOne(namespace + "findUser", user);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			MyBatisUtil.closeSession(session);
	...
	public int modifyUser(User user) {
		...
		int result = 0;
		try {
			result = session.update(namespace + "modifyUser", user);
			session.commit();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public int deleteUser(User user) {
		...
		int result = 0;
		try {
			result = session.delete(namespace + "deleteUser", user);
			session.commit();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static void main(String[] args) {
		UserDaoImpl uil = new UserDaoImpl();
		List<User> users =null;
		User us= new User();
		us.setUserName("zhangsandddd");
		users = uil.findAllUsers(us);
	}
